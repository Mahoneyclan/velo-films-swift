...
struct ElevationRenderer {
    ...
    static func render(flattenRows: [FlattenRow],
        ...
        // Position marker
        if let posIndex = flattenRows.firstIndex(where: { $0.gpxEpoch >= currentEpoch }) {
            let markerX = xPos(distances[posIndex])
            let markerY = yPos(flattenRows[posIndex].elevation)
            ctx.setFillColor(CGColor(red: 230/255, green: 175/255, blue: 0, alpha: 1))
            ctx.fillEllipse(in: CGRect(x: markerX - 3, y: markerY - 3, width: 6, height: 6))
        }
